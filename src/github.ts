...
export class GitHub {
  ...
  public static async writeBuildSummary(opts: BuildSummaryOpts): Promise<void> {
    ...
    if (summaryError) {
      ...
      if (Util.countLines(summaryError) > 10) {
        // prettier-ignore
        sum
          .addRaw(`<details ${Util.countLines(summaryError) <= 10 ? ' open' : ''}><summary><strong>Error</strong></summary>`)
            .addCodeBlock(he.encode(summaryError), 'text')
          .addRaw(`</details>`);
      } else {
        // prettier-ignore
        sum
